# 13. Native highlights own optimistic paint, and remove overlays retire on a different color
...
We keep the vapor fix and drop most of that cost. If the server reports a *different* color, the remove overlay retires. That color cannot be an echo of the deletion. It is newer data. The remaining failure needs the server two steps behind (green → yellow → remove, then a GET that still reports green).
