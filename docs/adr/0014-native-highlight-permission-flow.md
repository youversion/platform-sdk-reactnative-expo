...
- Consent-flow copy carries the Swift SDK's strings under React Native's **flat camelCase** key names (`signInIntroducing`, `dataExchangeHighlightsQuestion`, `cancel`, …). The dotted Swift spellings were the original draft here; `platform-localization` settled the question in favour of flat, matching the existing `react` surface, and this repo followed. The keys arrive through the localization sync into `packages/ui/src/i18n/locales/en.json`, which is generated and must never be hand-edited.
...
- `appName` in `signInParagraph` is derived from `expo-application`'s `applicationName` (`lib/app-name.ts`), not configured. Zero new provider props, correct by default. If Android's app label proves unreliable, that function is the single place an explicit provider prop replaces. It adds `expo-application` to `packages/ui`'s peer dependencies — no new native module for consumers, since core already required it.
