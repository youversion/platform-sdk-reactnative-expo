...
- Unknown values without `"` pass through unchanged. A consumer-supplied stack that contains `"` is URI-encoded so it can cross the iOS bridge; encoding runs on both platforms for a single code path.
