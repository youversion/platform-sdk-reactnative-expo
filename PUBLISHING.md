...
The UI package's `prepublishOnly` also **rewrites the SDK version into the tarball**: `scripts/stamp-sdk-version.cjs` replaces the `'Dev'` sentinel in the compiled `build/lib/sdk-version.js` with the real `package.json` version, so published builds report `ReactNativeSDK=<version>` in the `x-yvp-sdk` header while source builds keep reporting `Dev`. It runs after `expo-module build` and aborts the publish if the sentinel is missing or duplicated. See [ADR 0012](docs/adr/0012-sdk-version-stamp-on-publish.md).
...
That tarball reports `ReactNativeSDK=Dev` — expected, not a bug: `prepublishOnly` runs on `pnpm publish`, never on `pnpm pack`. To check the header instead of (or alongside) DOM rendering, pack a publish-identical tarball by running the publish steps by hand:
