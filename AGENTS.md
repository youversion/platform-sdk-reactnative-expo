...
- The hook re-fetches on mount, on a scope change, and when the app returns to the foreground. The foreground trigger is `background → active` **only** (`shouldRefetchOnForeground`, layer-1 tested): on iOS `expo-web-browser` parks the app in `inactive` for sign-in and consent, and those flows already re-fetch, so firing there would double-fetch every round-trip. It is deliberately stricter than the auth provider's unfiltered `active` listener — that one guards a nearly-free token refresh, this one a network GET. Android's Custom Tab is a separate activity, so a consent round-trip there legitimately arrives as `background → active` and may cost one extra GET; accepted, and idempotent.
- Navigation focus is **not** SDK-detected — that would force `@react-navigation/native` on every consumer as a peer. `BibleReader` accepts a `ref` (`BibleReaderHandle`) exposing `refreshHighlights()`, which is core's own `refresh`: never rejects, single-flight, safe to await for a `RefreshControl`. The recipe is `useFocusEffect` in the host; see `apps/example/app/(tabs)/index.tsx`.
