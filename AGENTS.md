...
- **Web content caveat (not a setup bug):** the app renders the Web SDK, which fetches Bible content directly from `api.youversion.com`. From a `localhost` browser origin those requests fail (401 + missing CORS headers) unless a **valid** `EXPO_PUBLIC_YOUVERSION_APP_KEY` is set in `apps/example/.env`. With an invalid/placeholder key the app shell (the Bible/VOTD/Card/Profile tab bar and navigation) still renders, but all content areas stay blank. Get a key from the [YouVersion Platform console](https://platform.youversion.com) and set it in `apps/example/.env` (gitignored; copy from `.env.example`). The `apps/example/README.md` mentions a stale `yvp-rn-example://callback`; the code + `app.json` actually use `youversionauth://callback`.
