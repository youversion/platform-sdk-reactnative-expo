...
- `useYVAuth().requestPermissions(permissions)` is the **just-in-time grant** (data exchange): a signed-in user grants a permission on the spot, no sign-out. Mint (`POST /data-exchange/token`, 201) → hosted consent in an auth session → parse the return → merge into the grant cache. Resolves to a `DataExchangeOutcome` (`granted` / `cancel` / `failure` with `reason: 'not-signed-in' | 'not-permitted' | 'user-changed' | 'transient'`) and never throws. Permission-generic — nothing highlights-specific lives in `auth/data-exchange.ts`.
  ...
  - An **initiator guard** fails closed: an `AuthIdentity` (`{ sessionId, userId }`) is captured before minting and re-read after the browser returns; any difference discards the grant (`reason: 'user-changed'`). `sessionId` is a local counter compared only for equality, not a server-issued value. Compare identity, not tokens — a mid-flow refresh is not a user change, which is why the session id moves only in `setIdentity` (sign-in and sign-out) and a token-only `setAuthState` leaves it alone. **Both halves are load-bearing**: `userId` alone is `null` for a signed-out user _and_ for a signed-in user whose id_token carried no `sub`, so an id-less user's grant would survive a mid-flow sign-out and be read back by the next id-less user; the session id alone would miss a change of id within one session. A same-session id-less user still passes, deliberately — failing closed there locks those users out of the flow entirely.
