...
- The swatch rule lives in `lib/verse-action-swatches.ts` (layer 1), ported verbatim from the Web SDK popover. It is an **ANY** rule, and Swift and Kotlin agree on the remove list. A partially-covering color appearing in both rows is intended.
...
- **Node 24 is mandatory** (`engines.node >=24`, `.nvmrc` = 24). It is provided via `nvm` (default alias is set to 24), so a fresh **login** shell picks up Node 24 + pnpm through `~/.bashrc` → nvm. The VM also has a shimmed `/exec-daemon/node` (v22) that wins in non-login shells; if `node --version` shows v22 or `pnpm` is "command not found", run `nvm use default` (or start a login shell with `bash -l`) before running commands. `pnpm` only exists inside the nvm Node bin — there is no system-wide pnpm.
- **Long-running dev servers** (Metro/Expo) must be run in a tmux session that has loaded nvm; otherwise they inherit the v22 shim.
- **This SDK is tested on Android/iOS, not web.** The team validates on native dev clients; web is not a real target for this RN Expo SDK (the separate React Web SDK covers web). Treat web here purely as a fallback smoke check — it cannot exercise the native bridge and, per the caveat below, does not even render the DOM-component Bible content. Prefer the automated suite (`pnpm test`) and a direct API check (below) as the meaningful validations available in this VM.
- **Only the web target can run on this headless Linux VM.** `cd apps/example && pnpm web` (`expo start --web`) bundles and serves at `http://localhost:8081`. The native dev-client flows (`pnpm build:ios` / `build:android`, `pnpm start`) need iOS/Android simulators that are **not available** here — do not attempt them.
- **Web target does not render Bible content — verify content via the API instead.** The Bible reader/VOTD/card are Expo DOM Components (`'use dom'`) that only paint inside a native WebView; on the web target their content area stays blank and makes no API calls (the app shell — Bible/VOTD/Card/Profile tabs + navigation — still renders). This is the DOM-on-web limitation, not a key/CORS problem: with a valid key the API returns 200 **with** `access-control-allow-origin: *`. To prove the key + data path work from the VM, hit the API the SDK uses directly, e.g. `curl -H "X-YVP-App-Key: $EXPO_PUBLIC_YOUVERSION_APP_KEY" https://api.youversion.com/v1/bibles/3034/passages/JHN.3.16` (host `api.youversion.com`, auth header `X-YVP-App-Key`; see `@youversion/platform-core` `YouVersionAPI.ts` / `client.ts`). Full in-app content rendering requires a native dev client (iOS/Android), which needs a simulator unavailable on this VM.
- **App key:** set a **valid** `EXPO_PUBLIC_YOUVERSION_APP_KEY` in `apps/example/.env` (gitignored; copy from `.env.example`) — get one from the [YouVersion Platform console](https://platform.youversion.com). EXPO_PUBLIC_ vars are inlined at bundle time, so restart Expo with `-c` after changing `.env`. The `apps/example/README.md` mentions a stale `yvp-rn-example://callback`; the code + `app.json` actually use `youversionauth://callback`.
- Standard lint/typecheck/test/build/run commands live in **Setup Commands** above and in `package.json` scripts — they all pass under Node 24 with no extra setup beyond `pnpm install`.
