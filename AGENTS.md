...
- **First-party exemption**: `minimumReleaseAgeExclude` carries `@youversion/*`. The cooldown exists to blunt a hijacked _third-party_ release; we control our own publishes, and this SDK routinely needs a same-day Web SDK version whenever `@youversion/platform-react-ui` ships an enabling change. The trade is real — a leaked npm token on the `@youversion` scope now lands immediately instead of in three days — and it is deliberate, not an oversight.
