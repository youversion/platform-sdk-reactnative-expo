...
- `useYVAuth().requestPermissions(permissions)` is the **just-in-time grant** (data exchange): a signed-in user grants a permission on the spot, no sign-out. Mint (`POST /data-exchange/token`, 201) → hosted consent in an auth session → parse the return → merge into the grant cache. Resolves to a `DataExchangeOutcome` (`granted` / `cancel` / `failure` with `reason: 'not-signed-in' | 'not-permitted' | 'user-changed' | 'transient'`) and never throws. Permission-generic — nothing highlights-specific lives in `auth/data-exchange.ts`.
  ...
  - An **initiator guard** fails closed: an `AuthIdentity` (`{ epoch, userId }`) is captured before minting and re-read after the browser returns; any difference discards the grant (`reason: 'user-changed'`). Compare identity, not tokens — a mid-flow refresh is not a user change, which is why the epoch moves only in `setIdentity` (sign-in and sign-out) and a token-only `setAuthState` leaves it alone. **Both halves are load-bearing**: `userId` alone is `null` for a signed-out user *and* for a signed-in user whose id_token carried no `sub`, so an id-less user's grant would survive a mid-flow sign-out and be read back by the next id-less user; the epoch alone would miss a change of id within one session. A same-epoch id-less user still passes, deliberately — failing closed there locks those users out of the flow entirely.
  ...
  - The return URL is the hardcoded, SDK-owned `youversionauth://callback` — **not** the app's `redirectUri`. **Android consumers must add `youversionauth` to their `app.json` `scheme` array** or the flow silently reports `cancel`; iOS needs nothing. See [ADR 0014](docs/adr/0014-data-exchange-return-scheme.md).
...
- **The branch point is the pre-flight `hasPermission('highlights')` cache read, not a write's 401/403.** Reason-first would burn a failed round-trip before every first highlight. A `reason: 'auth'` write is the _corrective_ path for a stale cache: it calls `invalidatePermissions()` and re-prompts **exactly once** (the reducer's `retried` flag), then goes terminal. Do not relax that bound — looping the hosted consent page on a repeated 403 is the worst possible reading of it.
- Pre-flight order is load-bearing: `await ensureFreshToken()` **before** reading the permission. An expired token 401s, a 401 classifies as `auth`, and `auth` reads as "stale grant" — so skipping the refresh makes an expired token present to the user as a request to grant a permission they already granted. `ensureFreshToken` is just the existing leeway-gated `refreshToken()` made public; it is a **workaround**, not a fix, for the bootstrap defect filed in `.claude/bugs/auth-provider-expired-access-token.md` (read the "partial mitigation" section before trusting it).
...
- The pending highlight is **in-memory only** (inside reducer state). `openAuthSessionAsync` returns to the same live process, so web's `sessionStorage` stash and TTL solve a problem native does not have. A scope change dispatches `RESET` during render (same "adjust state when props change" pattern as `use-highlights.ts`).
