...
- **Only the web target runs on this headless Linux VM.** `cd apps/example && pnpm web` (`expo start --web`) bundles and serves at `http://localhost:8081`. The native dev-client flows (`pnpm build:ios` / `build:android`, `pnpm start`) need iOS/Android simulators that are **not available** here — do not attempt them.
