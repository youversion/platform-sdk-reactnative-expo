...
const authWriteError = (failedVerses: number[], message = 'unauthorized'): HighlightWriteOutcome => ({
  status: 'error',
  reason: 'auth',
  message,
  failedVerses,
...
describe('the not-signed-in branch', () => {
  ...
  it('discards the pending highlight when sign-in is cancelled', () => {
    const state = run([TAP_SIGN_IN, { type: 'SIGN_IN_DONE', signedIn: false, hasPermission: false }])
...
describe('the signed-in-without-permission branch', () => {
  ...
  it('reaches applying with the original pending highlight intact', () => {
    const state = run([TAP_CONSENT, { type: 'CONFIRM' }, { type: 'GRANT_RESULT', outcome: granted() }])
