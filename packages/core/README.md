...
**Android apps must add `youversionauth` to their `app.json` `scheme` array** (alongside their own scheme) and rebuild the dev client — the flow returns to the SDK-owned `youversionauth://callback`, and without the scheme registered it reports `cancel`. iOS needs nothing extra.
