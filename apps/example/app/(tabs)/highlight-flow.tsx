  ...
  return (
    ...
    >
      ...
      <Section title="Auth" color={c}>
        <Row label="state" value={isLoading ? 'loading' : isAuthenticated ? 'signed in' : 'signed out'} color={c} />
        <Row label="user" value={userInfo?.name ?? userInfo?.id ?? '—'} color={c} />
        <Row label="granted" value={formatGrant(grantedPermissions)} color={c} />
        {isAuthenticated ? (
          <Pressable style={[styles.button, { borderColor: c.border }]} onPress={() => void signOut()}>
        ...
        <View style={styles.row}>
          {VERSES.map((verse) => {
            ...
            return (
              <Pressable
                key={verse}
                style={[styles.chip, { borderColor: c.border, backgroundColor: on ? c.chipOn : 'transparent' }]}
      ...
      <Section title="Flow" color={c}>
        ...
        <Row label="flowError" value={flow.flowError ? `${flow.flowError.reason}: ${flow.flowError.message}` : '—'} color={c} />
        <Row label="last outcome" value={lastOutcome === null ? '—' : JSON.stringify(lastOutcome)} color={c} />
      </Section>
...
const styles = StyleSheet.create({
  ...
  chip: { borderWidth: 1, borderRadius: 8, paddingVertical: 8, paddingHorizontal: 12, minWidth: 44 },
