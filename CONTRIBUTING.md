...
- **Distribution**: packages publish a compiled `dist/` build. Locally, Metro resolves TypeScript from source (`main` → `src/`); `publishConfig` swaps to `dist/` at publish time. `tsc` preserves `'use dom'` and Expo processes it from compiled files. See [ADR 0011](./docs/adr/0011-compiled-distribution.md).
