...
- **Commit messages**: every commit subject must be a valid [conventional commit](https://www.conventionalcommits.org/) (`type(scope): description`). Enforced by the `Commit Lint` workflow on every PR. The release pipeline reads commit subjects to compute the next version and the release notes — `chore(release):` commits are auto-ignored by the linter. See [RELEASING.md](./RELEASING.md) for the release flow and [RELEASE-RUNBOOK.md](./RELEASE-RUNBOOK.md) for recovery scenarios.
