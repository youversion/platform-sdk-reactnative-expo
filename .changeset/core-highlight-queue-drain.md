...
Queued highlight writes land on their own. A highlight parked while offline reaches the user's account as soon as service returns — no user action, and nothing on screen changing at the moment it does.
...
No public API changes of its own: no new exports, and no existing type or outcome changes meaning. A write that reports `{ status: 'queued' }` is owed, not permanent.
