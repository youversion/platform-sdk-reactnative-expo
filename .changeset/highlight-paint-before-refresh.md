...
Fix a highlight not painting until the token refresh in front of it finished. `useHighlightPermissionFlow.apply` awaited `ensureFreshToken()` before it reached the code that paints, so whenever a refresh was actually due — the access token at or inside its 60-second leeway, or a refresh already running from the `AppState` foreground listener — the user tapped a color and watched nothing happen for a full token round-trip.
