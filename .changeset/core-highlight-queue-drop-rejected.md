...
When a parked write comes back 401 or 403, the drain now forces a token refresh and states the write once more. The ordinary expired-token case is cured by that and lands normally. Only a **second** auth refusal, under a freshly minted token, drops the entry — the verse reverts to the color the server had, and un-paints on a reader that is still mounted, with no remount and no user action.

This closes the gap the drain shipped with (previously a definitive 401/403 was retried forever). It is reachable two ways: a consumer calling `useHighlights` directly without `useHighlightPermissionFlow`, or a `highlights` grant revoked between the tap and the drain.
